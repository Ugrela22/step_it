...
def create_json_file(path, file): 
    ...
    try:
        ...
        print(f"The path must by two-level... For example: 'files/json'")
        print("Exiting...")
        sys.exit()
    return file_path
...
def add_player(path, json_data: dict):
    players = read_json_file(path)

    if type(json_data['id']) != int:
        print("Players ID must be integer. Please try again...\n")
        return

    for player in players:
        if player['id'] == json_data['id']:
            print(f"Player with ID '{json_data['id']}' already exists...")
            return
    else:
